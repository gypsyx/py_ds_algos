...
def search_dll(dll: DoubleLinkedList, find) -> int:
    """
    Binary search for a dll using loop. Returns index if 'find' is found, -1 otherwise.
    """
    start = 0
    end = dll.count() - 1
...
def search_list(list_: list, find) -> int:
    """
    Binary search for a dll using loop. Returns index if 'find' is found, -1 otherwise.
    """
    start = 0
